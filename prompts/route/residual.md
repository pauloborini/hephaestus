...
Perguntas nascem **enfileiradas** — a cascata nunca pergunta nesta fase (D22). Cada pergunta registra `questionKey`, `contextFingerprint`, `fragmentId`, `reason`, `invalidates` e `blocking`. A fila é gravada em `.hephaestus/manifests/questions.json` e drenada por `interview` no lote inicial ou no único lote de revalidação permitido.
