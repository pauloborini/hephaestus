...
|-------|---------|---------|
| `meta` | `preflight`, `apply`, `verify(applied)` | `packVersion`, `schemaVersion`, `lastRunAt`, `lastRunId`, and `adoptionStatus` (`pending`/`applied`/`validated`) - versions, identity, and adoption lifecycle |
