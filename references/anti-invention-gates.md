...
|------|---------------|----------------------|------------|
| `preflight` | `mode` (`adopt`/`maintain`) + catálogo resolvido + checkpoint | presença/ausência de `.app-work/hephaestus-state.json`; git + worktree limpa; base `catalog/*` ± overlay `routing` | **path/pasta/texto**: inventar modo por heurística de árvore; inventar pasta via overlay; migrar campo desconhecido do state (D4) |
...
| `fragment` | `fragments.json` + `provenance[]` | unidades cortadas do snapshot; schema `fragment.schema.json` | **texto**: fragmentar sem proveniência; inventar enunciado que a fonte não traz |
...
| `reconcile` | `identity-map.json` (`action`/`decId`) | heading/fonte que enuncia a regra; inventário `max` (vivas + `## Histórico`); casamento por ID canônico ou enunciado | **DEC/texto**: inventar regra que a fonte não enuncia; reusar/renumerar `DEC-NNN`; cunhar de origem `.app-work/` (INV9); ID sem inventário |
| `interview` | bloco `answers` no state (fora da tx) | pergunta enfileirada com `questionKey`; resposta humana; territórios fechados | **pasta/path**: gravar pasta nova em `routing.overlay`; destino fora da lista fechada; promover pasta unknown a default de catálogo |
| `plan` | `plan.json` / `plan.md` | ledgers (`routing`, identity-map, questions); destrutividade explícita | **texto**: operação destrutiva `decidedBy: llm` sem `approved: true` (INV7); omitir remoção/relocate real |
...
| `verify_applied` | veredito applied / rollback | hash disco ≡ staging-manifest | **texto**: declarar válido com hash divergente; reverter `hephaestus-state.json` (exceção INV1) |
